...
export function registerGbpGetMediaTool(server: McpServer, user: UserContext): void {
  server.tool(
    "gbp_get_media",
    "Get photos and videos associated with a Google Business Profile location.",
    {
      location_name: z
        .string()
        .max(500)
        .describe('Full location path from gbp_list_locations (e.g., "accounts/123456/locations/789012")'),
      page_size: z
        .number()
        .min(1)
        .max(20)
    ...
    async (params) => {
      ...
      try {
        ...
        const rawMedia = await getGbpMedia(accessToken, params.location_name, params.page_size);
