...
export function registerGbpLocationOverviewTool(server: McpServer, user: UserContext): void {
  server.tool(
    ...
    async (params) => {
      ...
      try {
        ...
        if (reviewData.status === "fulfilled") {
          const { reviews, totalReviewCount, averageRating } = reviewData.value as any;
          reviewSummary = {
            total_reviews: totalReviewCount,
            average_rating: averageRating,
            recent_5: (reviews as any[]).slice(0, 5).map((r: any) => ({
              rating: r.starRating,
              date: r.createTime,
        ...
        try {
          ...
          if (accountMatch) {
            ...
            const loc = locId
              ? locations.find((l: any) => l.name.endsWith(locId))
              : locations[0];
            if (loc) {
              const addr = (loc as any).storefrontAddress;
              const addressParts = [
                ...(addr?.addressLines ?? []),
                addr?.locality,
              ...
              locationDetails = {
                ...
                phone: (loc as any).phoneNumbers?.primaryPhone,
                website: (loc as any).websiteUri,
                category: (loc as any).categories?.primaryCategory?.displayName,
              };
            }
          }
        } catch {
